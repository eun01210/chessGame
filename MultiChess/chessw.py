...
s_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # socket.AF_INET : IPv4 주소 사용, socket.SOCK_STREAM : 전송계층 프로토콜 TCP 사용
s_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) # 기존 포트 재사용
s_sock.bind((s_ip, s_port)) # 소켓에 서버 ip와 포트를 연결
s_sock.listen(10) # 백로그 크기(해킹 방지?)

print("상대를 기다리는 중입니다...")
client, c_addr = s_sock.accept() # 연결을 요청한 클라이언트 소켓 받아오기
print(c_addr, "가 연결되었습니다.") # 클라이언트 아이피 출력
...
while done : # done이 True일 경우 (무한 반복)
    ...
    if seq == "흑":
        ...
        rec = decry(client.recv(1024).decode('UTF-8'))
...
client.close() # 클라이언트 소켓을 종료한다.
s_sock.close() # 서버 소켓을 종료한다.
